...
{
	timer_remap[6] = timer_remap[4]; // clearing interrupt flags
	if (special)
	{
		top = (top * 50);
		top = (top / 51);
		 // changing overflow value for varying frequency
		timer_remap[7] = top;
	}

	// toggling dac channels
	dac_remap[8] = val;
	dac_remap[9] = val;
	if (val == 32)
		val = 0;
	...
	{
		period = 0;
		timer_remap[1] = 2; // stopping timer1
		special = 0;
	}
...
static void __exit sound_driver_cleanup(void)
{
	dac_remap[0] = 0x10;
	dac_remap[2] = 0;
	dac_remap[3] = 0;
...
module_exit(sound_driver_cleanup);
