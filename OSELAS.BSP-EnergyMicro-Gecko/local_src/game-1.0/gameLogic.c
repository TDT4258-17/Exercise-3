...
void movePlayerDiscrete(signed char dx, signed char dy) 
{
	...
	if (outOfBounds == 0)
	{
		int tileIndex = ((16*(pyNext/20))+(pxNext/20));
...
void movePlayerX(signed char dx)
{
	...
	if (outOfBounds == 0)
	{
		...
		short tile_c_x = 20*((pxNext + SPRITE_SIZE/2)/20);
		short tile_c_y = 20*((plr.py + SPRITE_SIZE/2)/20);

		int tileIndex_c = ((16*(tile_c_y/20))+(tile_c_x/20));
		unsigned char tileID_c = (map[tileIndex_c/2] >> 4*((tileIndex_c+1)%2)) & 0x0f;
...
void movePlayerY(signed char dy)
{
	...
	if (outOfBounds == 0)
	{
		...
		short tile_c_x = 20*((plr.px + SPRITE_SIZE/2)/20);
		short tile_c_y = 20*((pyNext + SPRITE_SIZE/2)/20);
...
void drawPlayerDiscrete()
{
	...
	unsigned short tile_ul_x = 20*(plr.pxOld/20);
	unsigned short tile_ul_y = 20*(plr.pyOld/20);

	unsigned int offset = tile_ul_x + 320*tile_ul_y;
